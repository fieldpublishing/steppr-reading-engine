...
import { deleteLocalDocument, listLocalDocuments, saveLocalDocument, type LocalDocument, type ParseProgress } from "@/lib/localStore";
...
export default function Library() {
  ...
  const [importProgress, setImportProgress] = useState<{ completed: number; total: number; fileName: string; phase: ParseProgress["phase"]; fraction: number }>();
  ...
  const importFiles = async (files: FileList | null) => {
    ...
    try {
      ...
      const imported: LocalDocument[] = [];
      for (let index = 0; index < queue.length; index += 1) {
        const file = queue[index];
        const document = await saveLocalDocument(file, (progress) => setImportProgress({ completed: index, total: queue.length, fileName: file.name, phase: progress.phase, fraction: progress.total ? progress.completed / progress.total : 0 }));
        imported.push(document);
        setImportProgress({ completed: index + 1, total: queue.length, fileName: file.name, phase: "saving", fraction: 1 });
      }
